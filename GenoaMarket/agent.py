...
class Agent:
    ...
    ind: int = field(default=-1)

    def make_buy_order(self, ri, pi, T):
        """
        ...
        Args:
            ...
            T :list[float] - the list of prices the agent will consider when making order.
        Returns:
        ...
        price_scale = self.price_limit_scale(T)
    ...
    def price_limit_scale(self, T):
        """
        Draws a scalar value from a normal distribution defined by mu=1.01 and
        sigma = k*std(T), where T is meant to be an array of log price returns.
        Since there are various possible edge cases for when len(T)<3, or maybe if
        an invalid scalar is drawn, this function is meant to control edge and
        special cases.

        Args:
            T :list - the array of log price returns this agent remembers.
        Returns:
        ...
        if len(T) < 3:
            if SMALL_HISTORY_WARNING:
                warnings.warn(
                    f"Agent {self.ind} is given to small of a history of log price returns (<3), will set sigma (pre scaling by k) to a default value"
                )
        ...
        if price_limit_scale < 0:
            ...
            if NEGATIVE_PRICE_LIMIT_SCALE_WARNING:
                warnings.warn(
                    f"Agent {self.ind} has drawn a negative value to determine price_limit_scale. Has been corrected to {price_limit_scale}"
                )
